...
const Dropdown = ({ value, onChange, children, ...props }: DropdownProps) => {
  ...
  const [selected, setSelected] = useState(
    selectedValue ? selectedValue.props.value?.toString() : ''
  );
  ...
  return (
    <div className='flex items-center'>
      {props.name === 'months' ? (
        <p className='text-base font-bold'>{props.caption}</p>
      ) : (
        <>
          <Select value={selected} onValueChange={handleChange}>
            <SelectTrigger className='relative w-full cursor-default rounded-md bg-blue-600 py-1.5 pr-10 pt-2 text-left shadow-sm ring-0 ring-inset focus:outline-none focus:ring-2 focus:ring-blue-600 sm:text-sm sm:leading-6'>
